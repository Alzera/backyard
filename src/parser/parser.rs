...
use super::{
  internal::{
    ...
    fors::ForParser,
    function::{ ConstructorParameterParser, FunctionParser, ParameterParser },
    global::GlobalParser,
    goto::GotoParser,
...
pub enum ParserInternal {
  ...
  ConstructorParameter(ConstructorParameterParser),
  Parenthesis(ParenthesisParser),
  Post(PostParser),
  Pre(PreParser),
  Program(ProgramParser),
...
impl ParserInternal {
  fn as_internal(&self) -> Box<&dyn Internal> {
    match self {
      ...
      ParserInternal::ConstructorParameter(x) => Box::new(x),
